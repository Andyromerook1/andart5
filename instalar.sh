...
if [ "$RAM_MB" -le 2200 ]; then
    ...
    N_PREDICT=180
    HISTORIAL_TURNOS=0
    echo "📦 Perfil detectado: RAM baja (~${RAM_MB}MB) → Qwen2.5-Coder-1.5B, ctx=2048, n_predict=180, sin memoria de turnos (necesario para que ande rápido en este hardware)"
